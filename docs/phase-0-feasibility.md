...
For #139 native-link evidence, the GitHub `Unreal BuildPlugin` workflow must be dispatched with `native_staging_mode=require` so it runs `python3 scripts/check_native_staging.py --require <Mac|Win64> --inspect-deps` before `RunUAT`. If the checkout does not already contain staged native binaries, `build_native_payload=true` must also be enabled so `scripts/stage_native_nozzle.py` builds and stages the payload from `deps/nozzle` first. The BuildPlugin log must contain the native payload build step and the `NozzleCore: WITH_NOZZLE_CORE=1` line from `NozzleCore.Build.cs`; after `RunUAT`, the workflow must also run the strict checker against `build/BuildPlugin/Nozzle-<target>/ThirdParty/nozzle` so the artifact is proven to contain the runtime payload. On Mac, the evidence run must additionally inspect `Binaries/Mac/*.dylib` with `scripts/check_buildplugin_module_dependencies.py` so the built Unreal runtime module is proven to reference packaged `ThirdParty/nozzle/lib/Mac/libnozzle.dylib` through loader paths/RPATHs rather than build-machine paths. A successful BuildPlugin run with the default placeholder staging mode is still only #142-style engine/package evidence.

If `RunUAT` is missing, that is a validation blocker. It must not be converted into a green static CI claim. If `RunUAT` succeeds, the package directory is still rejected unless `Nozzle.uplugin` exists, package-root `Native/` is absent, development `deps/` is absent, `Saved/` and `DerivedDataCache/` are absent, and the package matches the expected source or binary layout. Evidence runs must pass `--target-platform Win64` or `--target-platform Mac`; normal target-pinned evidence then requires a non-empty matching `Binaries/<target>/` directory even when `Source/` is present. Binary-only packages are also validated against the selected target instead of accepting a mislabeled artifact. Source-only acceptance requires explicit `--allow-source-only-artifact` and is diagnostic/archive review only, not #142 completion evidence. A real `RunUAT BuildPlugin` output may include package-root `Intermediate/`; that is artifact evidence, while committed repository/package-source outputs must remain free of generated build products. A no-target RunUAT call requires explicit `--allow-runuat-default-target` and is diagnostic-only, not acceptance evidence.
