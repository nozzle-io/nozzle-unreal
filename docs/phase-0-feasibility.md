...
If `RunUAT` is missing, that is a validation blocker. It must not be converted into a green static CI claim. If `RunUAT` succeeds, the package directory is still rejected unless `Nozzle.uplugin` exists, package-root `Native/` is absent, development `deps/` is absent, generated scratch directories are absent, and the package matches the expected source or binary layout. Evidence runs must pass `--target-platform Win64` or `--target-platform Mac`; normal target-pinned evidence then requires a non-empty matching `Binaries/<target>/` directory even when `Source/` is present. Binary-only packages are also validated against the selected target instead of accepting a mislabeled artifact. Source-only acceptance requires explicit `--allow-source-only-artifact` and is diagnostic/archive review only, not #142 completion evidence. A no-target RunUAT call requires explicit `--allow-runuat-default-target` and is diagnostic-only, not acceptance evidence.
