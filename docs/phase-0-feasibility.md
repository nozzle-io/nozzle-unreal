...
For #139 native-link evidence, the GitHub `Unreal BuildPlugin` workflow must be dispatched with `native_staging_mode=require` so it runs `python3 scripts/check_native_staging.py --require <Mac|Win64> --inspect-deps` before `RunUAT`. The BuildPlugin log must also contain the `NozzleCore: WITH_NOZZLE_CORE=1` line from `NozzleCore.Build.cs`; a successful BuildPlugin run with the default placeholder staging mode is still only #142-style engine/package evidence.
