...
| Unreal sender -> nozzle-viewer | Packaged Development | 320x240 | #140 PASS evidence: UE 5.7.4, macOS 15.4.1, Apple M4/Metal, packaged Development app bundle `org.nozzle-io.unreal-smoke`, no `com.apple.security.app-sandbox` entitlement, `unreal_metal_blit_to_iosurface`, `MTLPixelFormat=80`, `320x240`, IOSurface-backed intermediate with non-zero IOSurface ID, `usage=0x3`, `FRHIGPUFence` source sync before the cross-queue IOSurface copy, Metal intermediate blit `waitUntilCompleted`, and nozzle ring publish completion wait; `nozzle_enumerate_senders` showed `NozzleUnrealSmoke320` registered as backend Metal while the packaged app was running; runtime log contained no `SubmitAndBlockUntilGPUIdle` or `RHIBlockUntilGPUIdle`; viewer JSON proved 320x240, orientation/channel order, alpha patch `[255,0,255,64]`, moving marker, and five distinct frames. Build evidence: `BuildCookRun -platform=Mac -clientconfig=Development -build -cook -stage -package` reached `AutomationTool exiting with ExitCode=0 (Success)` in the local UE 5.7.4 environment, but local disk pressure interrupted Xcode's final CodeSign pass; the final app was re-signed with the repo-tracked `Samples/NozzleSmoke/Build/Mac/Resources/Sandbox.Server.entitlements` before runtime capture. Durable evidence: #140 https://github.com/nozzle-io/nozzle-dev/issues/140. Local artifact paths from that run: UE log `/private/tmp/nozzle_unreal_140_packaged_ue_320.log`, viewer JSON `/private/tmp/nozzle_unreal_140_packaged_viewer_320.json`. | PASS (#140) |
